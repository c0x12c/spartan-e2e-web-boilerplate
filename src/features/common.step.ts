...
import { getAuthPath, getTestDataByKey, loadData, trimUrl } from '../common/utils';
...
Given(
  ...
  async ({ scenarioData, basePage, loginPage, context, $testInfo }, userKey, dataName) => {
    ...
    if (fs.existsSync(authPath)) {
      ...
      await expect(loginPage.getPage()).toHaveURL(new RegExp(loginPage.getUrl()));

      // load auth data from file
      const storageStateData = JSON.parse(fs.readFileSync(authPath).toString('utf-8'));
      const localStorageAllData = storageStateData?.origins as Array<LocalStorageData>;
      const localStorageData = localStorageAllData.find(
        (localStorageByOrigin) => trimUrl(localStorageByOrigin.origin) == trimUrl(basePage.getUrl())
      );
      if (localStorageData) {
      ...
      // load data
      scenarioData.testData = loadData(dataName);
      const username = getTestDataByKey(scenarioData.testData, userKey + LOGIN_PAGE.KEY_USERNAME);
      const password = getTestDataByKey(scenarioData.testData, userKey + LOGIN_PAGE.KEY_PASSWORD);
...
When('I type text {string} to input {string}', async ({ basePage }, text, inputName) => {
